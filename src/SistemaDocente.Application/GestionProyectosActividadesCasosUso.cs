...
public sealed class GestionProyectosActividadesCasosUso
{
    ...
    public GestionProyectosActividadesCasosUso(
    ...
    {
        CargarGrupo(grupoId);
        var grados = CatalogoPlaneacionNem.NormalizarGradosObjetivo(entrada.GradosObjetivo);
    ...
    public IReadOnlyList<ProyectoResumen> ListarProyectosDelGrupo(GrupoId grupoId)
    {
        ...
        return _proyectos.ListarPorGrupo(grupoId)
            ...
            .Select(x => new ProyectoResumen(
                ...
                x.FechaTermino,
                x.Estado,
                _actividades.ListarPorProyecto(x.Id).Count,
                x.Version,
                x.Metodologia,
                x.GradosObjetivo))
            .ToArray();
    ...
    public ProyectoDetalle ActualizarProyecto(ProyectoId id, int version, EntradaProyecto entrada)
    {
        ...
        var grados = CatalogoPlaneacionNem.NormalizarGradosObjetivo(entrada.GradosObjetivo);
        ValidarActividadesDentroDeGradosProyecto(id, grados);
        proyecto.Actualizar(
            entrada.Nombre,
            entrada.Descripcion,
            entrada.FechaInicio,
    ...
    public ActividadProyectoDetalle PrepararNuevaActividad(
    ...
    public ActividadProyectoDetalle PrepararNuevaActividad(
        ...
        IReadOnlyCollection<GradoPrimaria>? gradosObjetivo)
    {
        var proyecto = CargarProyectoEditable(proyectoId);
        var grupo = CargarGrupo(proyecto.GrupoId);
        var grados = CatalogoPlaneacionNem.NormalizarGradosObjetivo(gradosObjetivo);
        CatalogoPlaneacionNem.ValidarGradosActividadDentroDelProyecto(proyecto.GradosObjetivo, grados);
        var estudiantes = SeleccionarEstudiantesObjetivo(grupo, grados);
    ...
    public ActividadProyectoDetalle CrearActividad(ProyectoId proyectoId, EntradaActividad entrada)
    {
        ...
        var grados = CatalogoPlaneacionNem.NormalizarGradosObjetivo(entrada.GradosObjetivo);
        CatalogoPlaneacionNem.ValidarGradosActividadDentroDelProyecto(proyecto.GradosObjetivo, grados);
        var estudiantesObjetivo = SeleccionarEstudiantesObjetivo(grupo, grados).ToHashSet();
        ValidarEntradas(entrada.Entregas, estudiantesObjetivo);
        var actividad = ActividadProyecto.Crear(
            proyecto.Id,
            proyecto.GrupoId,
            entrada.Titulo,
            entrada.Descripcion,
            entrada.FechaRealizacion,
            entrada.ObservacionesGenerales,
            proyecto.FechaInicio,
            proyecto.FechaTermino,
            estudiantesObjetivo,
    ...
    {
        var actividad = CargarActividad(id);
        return ProyectarActividad(actividad, CargarGrupo(actividad.GrupoId));
    }
    ...
    public IReadOnlyList<ActividadProyectoResumen> ListarActividadesDelProyecto(ProyectoId proyectoId)
    {
        ...
        return _actividades.ListarPorProyecto(proyectoId)
            .OrderBy(x => x.FechaRealizacion)
            .ThenBy(x => x.Titulo, StringComparer.Ordinal)
            .ThenBy(x => x.Id.Valor)
    ...
    {
        var actividad = CargarActividad(id);
        var proyecto = CargarProyectoEditable(actividad.ProyectoId);
        VerificarVersion(actividad.Version, version);
        var grados = CatalogoPlaneacionNem.NormalizarGradosObjetivo(entrada.GradosObjetivo);
        CatalogoPlaneacionNem.ValidarGradosActividadDentroDelProyecto(proyecto.GradosObjetivo, grados);
        ValidarCambioDeGradosSinReescribirPadron(actividad, grados);
        ...
        _actividades.Guardar(actividad, version);
        return ObtenerActividad(id);
    }

    public ActividadProyectoDetalle GuardarEntregasActividad(
        ActividadId id,
        int version,
        IReadOnlyCollection<EntradaEntregaActividad> entregas)
    {
        var actividad = CargarActividad(id);
        CargarProyectoEditable(actividad.ProyectoId);
        VerificarVersion(actividad.Version, version);
    ...
    private ProyectoDetalle ProyectarProyecto(ProyectoDidactico p) => new(
        p.Id,
        p.GrupoId,
        p.Nombre,
        p.Descripcion,
        ...
        p.Estado,
        p.Observaciones,
        _actividades.ListarPorProyecto(p.Id).Count,
        p.Version,
    ...
    private static ActividadProyectoDetalle ProyectarActividad(ActividadProyecto a, Grupo grupo)
    {
        ...
        return new(
            ...
            entregas.Count(x => x.EstadoEntrega == EstadoEntregaActividad.NoEntregada),
            a.Version,
            a.CampoFormativo,
            a.GradosObjetivo);
    }

    private static ActividadProyectoResumen ProyectarResumen(ActividadProyecto a) => new(
        a.Id,
        a.ProyectoId,
        a.Titulo,
        a.FechaRealizacion,
        ...
        a.Entregas.Count(x => x.EstadoEntrega == EstadoEntregaActividad.NoEntregada),
        a.Version,
        a.CampoFormativo,
        a.GradosObjetivo);
    ...
    private static DatosEntregaActividadRehidratada[] MapearValidandoHistorico(
    ...
    private static void ValidarEntradas(
        IReadOnlyCollection<EntradaEntregaActividad> entradas,
    ...
    private static EstudianteId[] SeleccionarEstudiantesObjetivo(
        Grupo grupo,
        IReadOnlyList<GradoPrimaria> gradosObjetivo)
    {
        if (gradosObjetivo.Count == 0)
        {
            return grupo.EstudiantesActivos.Select(x => x.Id).ToArray();
        }

        var grados = gradosObjetivo.ToHashSet();
        return grupo.EstudiantesActivos
            .Where(estudiante => grados.Contains(estudiante.Grado))
            .Select(estudiante => estudiante.Id)
            .ToArray();
    }

    private void ValidarActividadesDentroDeGradosProyecto(
        ProyectoId proyectoId,
        IReadOnlyList<GradoPrimaria> gradosProyecto)
    {
        if (gradosProyecto.Count == 0)
        {
            return;
        }

        foreach (var actividad in _actividades.ListarPorProyecto(proyectoId))
        {
    ...
    private static void ValidarCambioDeGradosSinReescribirPadron(
        ActividadProyecto actividad,
        IReadOnlyList<GradoPrimaria> nuevosGrados)
    {
        if (actividad.GradosObjetivo.SequenceEqual(nuevosGrados))
        {
            return;
        }

        throw new DomainConflictException(
            "Los grados objetivo de una actividad existente no pueden cambiarse porque su padrón histórico debe conservarse.");
    }
    ...
    {
        var p = CargarProyecto(id);
        if (p.Estado == EstadoProyecto.Finalizado)
            throw new DomainConflictException("El proyecto Finalizado es de sólo lectura.");
        return p;
    }
